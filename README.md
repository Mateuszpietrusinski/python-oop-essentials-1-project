...
- **2026-05-05** — Refaktoryzacja struktury pakietu `src/zoo/` do układu *jedna klasa = jeden plik*. Klasy zostały rozdzielone na sub-pakiety domenowe (`exceptions/`, `animals/`, `employees/`, `feeding/`), a moduły jednoklasowe (`enclosure.py`, `zoo.py`) pozostały płaskie. Plik `tests/test_zoo.py` został podzielony na cztery pliki dopasowane do capabilities (`test_animals.py`, `test_enclosure.py`, `test_feeding.py`, `test_zoo.py`) — łącznie nadal 15 testów. Publiczne API (`from zoo import …`) pozostaje bez zmian. Zaktualizowano sekcję *Struktura projektu*, tabelę *Mechanizmy OOP*, listę *Lokalizacja* w `CHECKLIST.md`, uzasadnienia **J9** i **J10** w `JUSTIFICATION.md`, treść `Projekt_A_Zoo_Garden.md` oraz aktywne specyfikacje OpenSpec. Zmiana udokumentowana jako proposal `restructure-one-class-per-file` (zarchiwizowany w `openspec/changes/archive/2026-05-05-restructure-one-class-per-file/`).
